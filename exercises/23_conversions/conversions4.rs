// `TryFrom` 是一种简单且安全的类型转换，在某些情况下可能以可控方式失败。
// 它基本上与 `From` 相同，主要区别是它应返回 `Result` 类型，而不是目标类型本身。
// 你可以在文档中了解更多信息：
// 文档链接：https://doc.rust-lang.org/std/convert/trait.TryFrom.html
...
fn main() {
    ...
    // 由于 `Color` 实现了 `TryFrom`，因此可以使用 `TryInto`。
    let c2: Result<Color, _> = [183, 65, 14].try_into();
    println!("{c2:?}");

    let v = vec![183, 65, 14];
    // 对于切片，应使用 `try_from` 函数
    let c3 = Color::try_from(&v[..]);
    println!("{c3:?}");
    // 或者将切片放在圆括号中并使用 `try_into`。
